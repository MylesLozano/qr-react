...
function CategoryDetails({
  ...
  if (!Array.isArray(items)) {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div
          className={`p-6 rounded-lg ${isDarkMode ? 'bg-red-900/50 text-red-200' : 'bg-red-100 text-red-700'}`}
        >
          <p>Error: Invalid items data</p>
          <Button onClick={onClose} color="gray" className="mt-4">
            Close
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      {' '}
      <div
        className={`w-full max-w-4xl max-h-[90vh] flex flex-col rounded-xl shadow-xl 
      ...
      >
        ...
        <div className="flex-1 overflow-hidden p-6">
          {isLoading ? (
            ...
            <div
              className={`flex items-center justify-center h-full min-h-[200px] rounded-lg 
