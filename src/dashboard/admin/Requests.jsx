...
const RequestDetailsModal = ({ request, onClose, isDarkMode }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isDarkMode ? 'bg-black/70' : 'bg-gray-500/70'}`}
    >
      <div
        className={`w-full max-w-2xl rounded-lg shadow-xl p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
      >
  ...
  const EmptyRequestsOptions = ({ isDarkMode, onExportSample, onToggleFilter }) => {
    return (
      <div className={`rounded-lg shadow-md p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} mb-6`}>
        ...
        <p className="mb-4">
          There are currently no requests that match your filters, or no requests have been
          submitted yet.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            <Button onClick={onToggleFilter} color="blue" size="md" className="w-full">
              Show All Requests
            </Button>
          </div>
          ...
          <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            <Button onClick={onExportSample} color="green" size="md" className="w-full">
              Download Sample
            </Button>
          </div>
        </div>
        ...
        <div
          className={`p-4 rounded-lg ${isDarkMode ? 'bg-indigo-900/20' : 'bg-indigo-50'} border ${isDarkMode ? 'border-indigo-800' : 'border-indigo-200'}`}
        >
      ...
      return (
        <div
          ...
          className={`border-b p-4 transition-colors duration-200 ${
            isSelected ? (isDarkMode ? 'bg-gray-700' : 'bg-blue-50') : ''
          }`}
          role="row"
      ...
      <div className={`p-6 ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {/* Filters */}
            <div
              className={`p-4 rounded-lg mb-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-md`}
            >
